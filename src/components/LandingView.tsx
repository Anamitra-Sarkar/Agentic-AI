      ...
      <div className="flex-1 relative overflow-y-auto custom-scrollbar pb-64">
         <nav className="p-8 flex justify-between items-center sticky top-0 bg-[#f7f6f2]/80 backdrop-blur-[12px] z-20 border-b border-alpha transition-all duration-300">
            <h1 className="text-2xl font-bold tracking-tight cursor-pointer flex items-center gap-3 font-display">
              <div onClick={() => {}} className="flex items-center gap-3"><span className="w-6 h-6 text-[#01696f]">■</span> AI Architect</div>
            </h1>
            <div className="flex gap-4 items-center">
              ...
              <button onClick={() => {}} className="text-sm font-bold px-6 py-2.5 bg-[#f9f8f5] border border-alpha hover:bg-[#efebe3] shadow-warm rounded-[8px] premium-transition">New Project</button>
            </div>
         </nav>
               ...
               <div className="max-w-4xl mx-auto bg-[#f9f8f5] p-8 rounded-[12px] shadow-[0_20px_50px_rgba(0,0,0,0.06)] border border-alpha flex flex-col gap-6 relative stagger-fade-in">
                 ...
                 {status === 'idle' && (
                   <div className="flex flex-col gap-6 w-full">
                     {!isUrlMode ? (
                       <>
                         <textarea className="flex-1 w-full p-6 bg-[#f7f6f2] border border-alpha rounded-[8px] focus:ring-2 focus:ring-[#01696f]/20 focus:border-[#01696f] focus:outline-none resize-none transition-all placeholder:text-[#6b6b6b]/50 text-sm font-medium" placeholder="Describe your next massive project..." value={prompt} onChange={(e) => setPrompt(e.target.value)} rows={3} />
                       ...
                       <div className="space-y-6">
                         <div className="flex flex-col sm:flex-row gap-4">
                           <div className="flex-1 relative">
                             ...
                             <input type="text" placeholder="https://example.com — paste any website" className="w-full pl-14 pr-6 py-4 bg-[#f7f6f2] border border-alpha rounded-[8px] focus:ring-2 focus:ring-[#01696f]/20 focus:border-[#01696f] focus:outline-none text-sm font-medium" value={cloneUrl} onChange={(e) => setCloneUrl(e.target.value)} />
                           </div>
